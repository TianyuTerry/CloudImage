...
LR_acc2=CVmaster(generic_fun="logistics",X,y,K,loss_fun=c("accuracy","prf","auc"),drop_margin0=TRUE)
QDA_acc1=CVmaster(generic_fun="QDA",X,y,K,loss_fun=c("accuracy","prf","auc"),drop_margin0=FALSE)
QDA_acc2=CVmaster(generic_fun="QDA",X,y,K,loss_fun=c("accuracy","prf","auc"),drop_margin0=TRUE)
LDA_acc1=CVmaster(generic_fun="LDA",X,y,K,loss_fun=c("accuracy","prf","auc"),drop_margin0=FALSE)
LDA_acc2=CVmaster(generic_fun="LDA",X,y,K,loss_fun=c("accuracy","prf","auc"),drop_margin0=TRUE)
NB_acc1=CVmaster(generic_fun="NB",X,y,K,loss_fun=c("accuracy","prf","auc"),drop_margin0=FALSE)
NB_acc2=CVmaster(generic_fun="NB",X,y,K,loss_fun=c("accuracy","prf","auc"),drop_margin0=TRUE)
CART_acc1=CVmaster(generic_fun="CART",X,y,K,loss_fun=c("accuracy","prf","auc"),drop_margin0=FALSE)
CART_acc2=CVmaster(generic_fun="CART",X,y,K,loss_fun=c("accuracy","prf","auc"),drop_margin0=TRUE)
```
...
```
```{r}
# dim1 <- seq(from = -8, to = 12, length = 100)
# dim2 <- seq(from = -8, to = 5, length = 100)
# low_dim <- as.matrix(expand.grid(dim1, dim2))
# 
# means <- colMeans(train_x)
# means_mat <- matrix(means, nrow = 10000, ncol = 3, byrow = TRUE)
# sds <- apply(train_x, 2, sd)
# sd_mat <- matrix(sds, nrow = 10000, ncol = 3, byrow = TRUE)
# data_high <- low_dim %*% t(trn_pca$svd$V[, 1:2]) * sd_mat + means_mat
# data_high <- data.frame(data_high)
# colnames(data_high) <- rownames(trn_pca$var$contrib)
# preds_proj <- sign(predict(LR_acc1$model, newdata = data.frame(data_high), type="response")-0.5)
# 
# plot_boundary <- data.frame(low_dim)
# colnames(plot_boundary) <- c("Dim.1", "Dim.2")
# 
# plot_boundary |> 
#   tibble() |> 
#   mutate(label = preds_proj) |> 
#   ggplot(aes(x = Dim.1, y = Dim.2, color = as.factor(label))) +
#   geom_point(alpha = 0.5, size=0.1) +
#   geom_point(data = proj, shape = 18, size = 0.01,alpha=0.1) +
#   labs(
#     x = "PC1", y = "PC2", title = "PCA Projection of LDA Decision Boundary"
#     ) +
#   theme_bw()
```

```{r}
# data_pca=as.data.frame(cbind(pca_prcomp$x[,1:2],
#                              train_data$label,
#                              sign(LR_acc1$score-0.5)))
# colnames(data_pca)=c("PC1","PC2","label","pred")
# data_pca_eval=data_pca%>%mutate(eval=case_when(pred==label&label==1~"TP",
#                                                  pred==label&label==-1~"TN",
#                                                  pred!=label&pred==1~"FP",
#                                                  TRUE~"FN"))
# NDAI=seq(min(train_x$NDAI),max(train_x$NDAI),length.out=300)
# CORR=seq(min(train_x$CORR),max(train_x$CORR),length.out=300)
# # boundary=expand.grid(NDAI,CORR)
# boundary=data.frame(NDAI,CORR)
# colnames(boundary)=c("NDAI","CORR")
# boundary$SD=SD=(-coef(model)[1]-coef(model)[2]*NDAI-coef(model)[3]*CORR)/coef(model)[4]
# boundary=boundary%>%filter(SD>min(train_x$SD)&SD<max(train_x$SD))
# pca_boundary=as.data.frame(predict(pca_prcomp,boundary)[,1:2])
# colnames(pca_boundary)=c("PC1","PC2")
# data_pca_eval%>%
#   ggplot(aes(x=PC1,y=PC2))+
#   geom_point(aes(color=eval),size=0.01)+
#   geom_point(data=pca_boundary,aes(x=PC1,y=PC2),color="black")+
...
test_data$pred=sign(NB_acc1$prob-0.5)
...
test_data_qda$pred=sign(QDA_acc1$prob-0.5)
...
test_data_lda$pred=sign(LDA_acc1$prob-0.5)
