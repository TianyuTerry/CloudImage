...
3(a)
```{r}
a=rbind(LR_acc1$perf,LR_acc2$perf,
        QDA_acc1$perf,QDA_acc2$perf,
        LDA_acc1$perf,LDA_acc2$perf)%>%
  data.frame()%>%
...
```{r}
perf.all=data.frame()
split1.list=c(LR_acc1,QDA_acc1,LDA_acc2)
split1.name=c("LR", "QDA", "LDA")
for(i in seq_along(split1.list)){
  res=split1.list[i]
  pred=prediction(res$prob,res$gold)
  perf=performance(pred,"tpr","fpr")
  model.perf=data.frame(slot(perf, "x.name")=slot(perf, "x.values"),
                        slot(perf, "y.name")=slot(perf, "y.values"),
                        slot(perf, "alpha.name")=slot(perf, "alpha.values"),
                        model=split1.name[i])
...
```{r}
slot(perf, "alpha.name")
slot(perf, "alpha.values")
```
