...
```{r labeled-images,fig.cap="Image1-3 with expect labelled.",echo=FALSE}
...
```{r label-feature1,fig.cap="Scattor-plots of label verse each feature, with unlabelled data removed. Correlations of label and each feature with and without unlabeled data. ", echo=FALSE}
...
To enlarge the gap between the features of not cloud data and features of cloud data, we drop blocks which are not purely not cloud or cloud. it means that we drop almost all data points around the boundaries (in Figure \@ref(fig:labeled-images)) between different labels, and everything else is the same as the first method.
...
```{r best-features,fig.cap="Result of logistic regression forward selection on all features." ,out.width="50%",echo=FALSE}
